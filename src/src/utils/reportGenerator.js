...
export async function generatePDF(reportType, dateRange = {}) {
  try {
    ...
    const drawHeader = (title, sub='') => {
      doc.setFillColor(26,54,93); doc.rect(0,0,W,30,'F')
      doc.setFillColor(43,82,160); doc.rect(0,28,W,2,'F')
      doc.setFont('helvetica','bold'); doc.setFontSize(16); doc.setTextColor(255,255,255)
      doc.text('CanHoa', 14, 13)
      doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(147,174,230)
      doc.text('HOA Management Platform', 14, 19)
      doc.setFontSize(12); doc.setFont('helvetica','bold'); doc.setTextColor(255,255,255)
      doc.text(title, W-14, 12, {align:'right'})
      doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(197,212,242)
      doc.text(sub || periodLabel, W-14, 19, {align:'right'})
      doc.setFillColor(240,245,255); doc.rect(0,30,W,12,'F')
      doc.setFontSize(8.5); doc.setFont('helvetica','bold'); doc.setTextColor(26,54,93)
      doc.text(COMMUNITY.name, 14, 38)
      doc.setFont('helvetica','normal'); doc.setTextColor(74,95,138)
    ...
    const drawFooter = (page, total) => {
      ...
      doc.setFontSize(7); doc.setFont('helvetica','normal'); doc.setTextColor(74,95,138)
    ...
    const drawKPIs = (y, items) => {
      ...
      items.forEach((it,i)=>{
        const x = 14+i*(bw+4)
        doc.setFillColor(240,245,255); doc.roundedRect(x,y,bw,20,2,2,'F')
        doc.setDrawColor(197,212,242); doc.roundedRect(x,y,bw,20,2,2,'S')
        doc.setFontSize(13); doc.setFont('helvetica','bold'); doc.setTextColor(26,54,93)
    ...
    const TS = {
      theme:'grid',
      headStyles:{fillColor:[15,32,64],textColor:255,fontStyle:'bold',fontSize:8.5},
      bodyStyles:{fontSize:8,textColor:[15,31,20]},
      alternateRowStyles:{fillColor:[240,253,244]},
      footStyles:{fillColor:[15,32,64],textColor:255,fontStyle:'bold',fontSize:8.5},
    ...
    if (reportType === 'income') {
      ...
      doc.setFontSize(9); doc.setFont('helvetica','bold'); doc.setTextColor(26,54,93)
    ...
    // ── RESERVE FUND ──
      ...
      y = doc.lastAutoTable.finalY+6
      doc.setFillColor(240,245,255); doc.setDrawColor(197,212,242)
      doc.roundedRect(14,y,W-28,14,2,2,'FD')
      doc.setFontSize(7.5); doc.setFont('helvetica','normal'); doc.setTextColor(26,54,93)
    ...
    // ── ANNUAL REPORT ──
      ...
      doc.setFont('helvetica','normal'); doc.setTextColor(74,95,138); doc.setFontSize(8.5)
