...
export function ResidentDashboard() {
  ...
  return (
    <div>
      ...
      {/* Balance Card */}
      <div style={{ marginBottom:24, padding:28, background:balance>0?'linear-gradient(135deg,#dc2626 0%,#b91c1c 100%)':'linear-gradient(135deg,#1A365D 0%,#0f2040 100%)', borderRadius:20, color:'white', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:16 }}>
          ...
          {balance>0 && (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <button className="btn btn-xl" style={{ background:'white', color:'#1A365D', fontWeight:700 }} onClick={()=>setPayModalOpen(true)}>
      ...
      <div className="card" style={{ marginBottom:24, padding:20 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px,1fr))', gap:10 }}>
          {[
            { label:'Pay Dues', icon:CreditCard, color:'#1A365D', action:()=>setPayModalOpen(true) },
  ...
  const downloadStatement = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        ...
        headStyles:{fillColor:[15,32,64],textColor:255,fontStyle:'bold'},
        alternateRowStyles:{fillColor:[240,253,244]},
        footStyles:{fillColor:[15,32,64],textColor:255,fontStyle:'bold'},
      })
    ...
    <div>
      ...
      <div className="grid-4" style={{ marginBottom:24 }}>
        {[
          ...
          { label:'Resolved', value:myRequests.filter(r=>r.status==='Resolved').length, color:'#2b52a0' },
        ].map(s=>(
...
export function ResidentDocuments() {
  ...
  const ICON_COLOR = { PDF:'#ef4444', Excel:'#2b52a0', Word:'#2563eb' }
